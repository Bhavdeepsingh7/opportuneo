...
def test_create_order_uses_server_price_in_paise():
    ...
    with patch("httpx.AsyncClient.post", AsyncMock(return_value=mock_response)), \
         ...
         patch("app.routers.payments.settings.razorpay_key_secret", "rzp_test_secret"):
...
def test_verify_payment_activates_subscription():
    ...
    from app.dependencies import get_current_user
    app.dependency_overrides[get_current_user] = lambda: USER
    with patch("app.routers.payments._verify_payment_signature", return_value=True), \
         patch("app.routers.payments._fetch_order", AsyncMock(return_value=mock_fetch.json.return_value)), \
...
def test_payment_captured_webhook_activates_subscription():
    ...
    with patch("app.routers.payments.settings.razorpay_webhook_secret", "webhook_secret"), \
         patch("app.routers.payments._verify_webhook_signature", return_value=True), \
         patch("app.routers.payments._fetch_order", AsyncMock(return_value=mock_fetch.json.return_value)), \
